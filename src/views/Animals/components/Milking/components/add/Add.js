...
import { Card, CardContent, Fab, Grid, Divider, CircularProgress, TextField, colors, Button, CardActions, Box, Typography, Switch } from '@material-ui/core';
import { getLookups, postMilking, getParametersLimitAll, getParametersLocalSettingsOrgAll,genericFunctionSixParameters, genericFunctionFourParameters } from '../../../../../../utils/API';
...
const Edit = props => {
  ...
  useEffect(() => {
    ...
    */
    ...
    (async (endpoint,desc,option,id, milk_date,lactation_number) => {
      await genericFunctionSixParameters(endpoint,desc,option, id, milk_date,lactation_number)
        .then(response => {
    ...
    })(endpoint_milking_parameter,'milk params',0, animal_id, moment(new Date()).format('YYYY-MM-DD'),null);
  ...
  // colostrum_milk_period
  let colostrum_milk_period = limitParameters.filter(obj => obj.category === 'colostrum_milk_period');
  let colostrum_milk_period_status = false;
  let colostrum_milk_period_value = 0;

  if (colostrum_milk_period.length > 0) {
    colostrum_milk_period_status = colostrum_milk_period[0].is_active_id;
    colostrum_milk_period_value = colostrum_milk_period[0].max_value;
  }
  ...
  const handleChange = event => {
    ...
    if (event.target.name === 'milk_date') {
      ...
      })(endpoint_milking_parameter,'milk params',0, animal_id, event.target.value,null);
    }
  };
  ...
  const handleSubmit = event => {
    ...
    (async (endpoint, id, values, user_id, quality_toggle, lactation_id, lactation_number, days_in_milk, testday_no) => {
      await postMilking(endpoint, id, values, user_id, quality_toggle, lactation_id, lactation_number, days_in_milk, testday_no)
        .then((response) => {
    ...
    })(endpoint_milking_add, animal_id, values, user_id, quality_fields_view, milkingParameters.lactation_id, milkingParameters.lactation_number, milkingParameters.days_in_milk, milkingParameters.testday_no);
  };
                      ...
                      >
                        ...
                        >
                          <TextField
                            ...
                            inputProps={{
                              ...
                              min: (colostrum_milk_period_status) ? moment(milkingParameters.calving_date).add(parseInt(colostrum_milk_period_value), 'days').format('YYYY-MM-DD') : moment(milkingParameters.calving_date).format('YYYY-MM-DD')
                            }}
                            required
                            label="Milk Date"
                            type="date"
                        ...
                        >
                          <TextField
                            ...
                            variant="outlined"
                            type="number"
                            value={milkingParameters.testday_no}
                            required
                          />
                        </Grid>
