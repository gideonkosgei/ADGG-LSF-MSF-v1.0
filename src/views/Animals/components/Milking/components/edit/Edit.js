...
import { getLookups, updateMilking, getMilkingByEventId, getParametersLimitAll, getParametersLocalSettingsOrgAll, genericFunctionSixParameters } from '../../../../../../utils/API';
import { endpoint_lookup, endpoint_milking_update, endpoint_milking_specific, endpoint_parameter_limit_all, endpoint_parameter_local_settings_org_all, endpoint_milking_parameter } from '../../../../../../configs/endpoints';
        ...
        await genericFunctionSixParameters(endpoint, desc, option, id, milk_date, lactation_number)
          .then(response => {
            setValues({
              ...
              "days_in_milk": response.payload[0][0].days_in_milk,
              "testday_no": response.payload[0][0].testday_no
            });
          });
      })(endpoint_milking_parameter, 'milk params', 1, values.animal_id, event.target.value, values.lactation_number);
    }
  };
                  ...
                  >
                    <TextField
                      ...
                      inputProps={{
                        ...
                        min: (colostrum_milk_period_status) ? moment(values.calving_date).add(parseInt(colostrum_milk_period_value), 'days').format('YYYY-MM-DD') : moment(values.calving_date).format('YYYY-MM-DD')
                      }}
                      required
                      label="Milk Date"
                      type="date"
