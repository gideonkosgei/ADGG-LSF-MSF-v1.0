...
import {endpoint_batch_sync_upload} from '../../../../../../../../configs/endpoints';
  ...
  const handleSubmit = event => {
    ...
    (async  (endpoint,rows,cols,user_id,org_id,uuid) => {     
      ...
      .then(() => {  
        ...
        setTimeout(function() {
          window.location.replace(`/batch-process/sync/add/${batch_uuid}`);
        }, delayInMilliseconds);
        document.forms["event"].reset();
      }).catch((err) => {        
        setopenSnackbarError(true); 
      });
    })(endpoint_batch_sync_upload,rows,cols,user_id,organization_id,batch_uuid);    
  };
